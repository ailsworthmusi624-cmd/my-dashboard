...
import {
  ...
  CheckCircle2, X, Menu, Bell, LayoutDashboard, PieChart, ArrowUpRight,
...
// Declare optional globals injected by the hosting environment
...
// Prevent re-initializing if HMR reloads the module
const firebaseApp = getApps().length === 0 ? initializeApp(firebaseConfig) : getApps()[0];
const auth = getAuth(firebaseApp);
const db = getFirestore(firebaseApp);
const appId =
  typeof __app_id !== 'undefined' ? __app_id : 'dashboard-27927';
...
type ActiveTab = 'dashboard' | 'calendar' | 'analytics' | 'investing';
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: 20, background: '#fee', color: '#c00', fontFamily: 'sans-serif' }}>
          <h2>Упс! Произошла ошибка в коде:</h2>
...
const makeInitialDebts = (): Debt[] => [
  {
    ...
    rate: 25.9,
    minPayment: 3500,
    nextPaymentDate: new Date(Date.now() + 5 * 86400000).toISOString().split('T')[0],
    isPaidThisMonth: false,
    details: { summary: 'Важно: Снятие наличных отменяет грейс-период.' },
  },
  {
    id: 2,
    name: 'ВТБ (Кредитка)',
    ...
    rate: 29.9,
    minPayment: 5000,
    nextPaymentDate: new Date(Date.now() + 12 * 86400000).toISOString().split('T')[0],
    isPaidThisMonth: false,
    details: { summary: 'Минимальный платеж 3% от суммы долга.' },
  },
];
...
export default function App() {
  ...
  const [freeMoney, setFreeMoney] = useState<number>(15000);
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        // Still mark loading done so UI shows
        setIsLoading(false);
      }
    };
    initAuth();
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          setStrategy((data.strategy as Strategy) || 'avalanche');
        } else {
          const initial = makeInitialDebts();
          setDoc(docRef, { debts: initial, freeMoney: 15000, strategy: 'avalanche' });
        }
        setIsLoading(false);
      },
      (err) => {
        console.error('Ошибка загрузки данных:', err);
        // Fallback to demo data so app is still usable
        setDebts(makeInitialDebts());
        setIsLoading(false);
      }
    );
  ...
  // --- CLOUD UPDATE ---
  const updateCloud = (newDebts: Debt[], newFreeMoney: number, newStrategy: Strategy) => {
    if (!user) return;
    const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'appState', 'main');
  ...
  const sortedDebts = useMemo(
    () =>
      [...debts].sort((a, b) => {
        ...
        return new Date(a.nextPaymentDate || 0).getTime() - new Date(b.nextPaymentDate || 0).getTime();
      }),
    [debts]
  );

  const notifications = useMemo(() => {
    const alerts: { id: string; type: 'error' | 'warning'; title: string; text: string }[] = [];
    ...
    debts
      .filter(d => !d.isPaidThisMonth && getDaysDiff(d.nextPaymentDate) >= 0 && getDaysDiff(d.nextPaymentDate) <= 3)
      .forEach(d => {
    ...
    return alerts;
  }, [debts, overdueDebts]); // eslint-disable-line react-hooks/exhaustive-deps

  const strategyAllocation = useMemo(() => {
    let remainingMoney = Number(freeMoney) || 0;
    const allocation: Record<number, number> = {};
    const targetDebts = debts.filter(d => d.balance > 0);
  ...
  const handleMarkPaid = (id: number) => {
    const newDebts = debts.map(d => {
      if (d.id === id) {
        return { ...d, isPaidThisMonth: true, balance: Math.max(0, d.balance - (d.minPayment || 0)), _prevBalance: d.balance };
      }
      return d;
    });
    setDebts(newDebts);
  ...
  const handleAddDebt = (e: React.FormEvent) => {
    ...
    const debtToAdd: Debt = {
      ...
      nextPaymentDate: newDebt.nextPaymentDate,
      isPaidThisMonth: false,
    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white rounded-[24px] p-5 md:p-6 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] flex flex-col justify-between relative">
          ...
          <div className="mt-4 text-xs text-gray-500">Из {formatMoney(totalMinPaymentAll)} в этом месяце</div>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-5 md:p-6 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] flex flex-col justify-between">
          ...
          <div className="relative mt-2">
            <input
              type="number"
              value={freeMoney}
              onChange={e => setFreeMoney(Number(e.target.value))}
          ...
          <div className="mt-4 text-xs text-gray-500">₽ для стратегии досрочки</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-[24px] p-6 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            ...
            <button
              ...
              className="text-xs font-medium text-gray-500 border border-gray-200 px-3 py-1.5 rounded-full hover:bg-gray-100 hover:text-gray-900 transition-colors flex items-center gap-1.5 w-max"
            >
          ...
          <div className="space-y-4">
            {sortedDebts.map(debt => {
              ...
              return (
                <div
                  key={debt.id}
                  className={`flex flex-col rounded-2xl border transition-all ${debt.isPaidThisMonth ? 'border-gray-100 bg-gray-50/50 opacity-70' : 'border-gray-100 hover:shadow-md hover:border-gray-200'}`}
                >
                  <div
                  ...
                  {expandedId === debt.id && (
                    <div
                      className="p-4 border-t border-gray-100 bg-[#F8FAFC] rounded-b-2xl cursor-default"
                    ...
                    >
                        ...
                        {Number(debt.rate || 0) > 0 && (
                          <div className="pt-4 border-t border-gray-50">
                            <span className="block text-xs text-gray-500 font-medium mb-2">
                              Куда уходит ваш платеж ({formatMoney(debt.minPayment)}):
                            </span>
                            <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden flex shadow-inner">
                              <div className="bg-red-400 h-full transition-all duration-500" style={{ width: `${interestPercent}%` }} />
                              <div className="bg-[#106A3C] h-full transition-all duration-500" style={{ width: `${principalPercent}%` }} />
                            </div>
                            <div className="flex justify-between mt-2 text-[11px] font-medium">
                              <span className="text-red-500 flex items-center gap-1">
                                <div className="w-2 h-2 rounded-full bg-red-400" />{formatMoney(monthlyInterest)} (Проценты)
                              </span>
                              <span className="text-[#106A3C] flex items-center gap-1">
                                <div className="w-2 h-2 rounded-full bg-[#106A3C]" />{formatMoney(principalPayment)} (В счет долга)
                              </span>
                            </div>
                          </div>
                        )}
                      ...
                      <h5 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                        <FileText size={16} className="text-gray-400" /> Условия договора
                      </h5>
        ...
        <div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] flex flex-col">
          <h3 className="text-lg font-bold text-gray-900 mb-6">Стратегия</h3>
          ...
          <div className="flex-1 flex flex-col justify-between">
            ...
            <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 mt-auto">
              ...
              <p className="text-xs text-gray-500 text-center mb-4">
                Оплачено {formatMoney(paidThisMonthAmount)} из {formatMoney(totalMinPaymentAll)}
              </p>
  ...
  const renderCalendar = () => {
    const calendarDebts = [...debts].sort(
      (a, b) => new Date(a.nextPaymentDate || 0).getTime() - new Date(b.nextPaymentDate || 0).getTime()
    );
    return (
          ...
          {calendarDebts.map(debt => {
            ...
            return (
              <div key={`cal_${debt.id}`} className="relative pl-6">
                <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white ${debt.isPaidThisMonth ? 'bg-emerald-500' : isPast ? 'bg-red-500' : 'bg-[#106A3C]'}`} />
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                  <div>
                    <span className={`text-sm font-bold ${isPast && !debt.isPaidThisMonth ? 'text-red-500' : 'text-[#106A3C]'}`}>
                      {formattedDate}
                    </span>
  ...
  const renderAnalytics = () => {
    const totalInterestPerMonth = debts.reduce(
      (sum, debt) => sum + (debt.rate > 0 ? (Number(debt.balance) * (Number(debt.rate) / 100)) / 12 : 0),
      0
    );
    const ratedDebts = debts.filter(d => d.rate > 0);
    const averageRate = ratedDebts.length
      ? ratedDebts.reduce((sum, d) => sum + Number(d.rate), 0) / ratedDebts.length
      : 0;
            ...
            {[...debts].sort((a, b) => b.balance - a.balance).map(debt => (
              <div key={`stat_${debt.id}`}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-bold text-gray-900">{totalDebt > 0 ? Math.round((debt.balance / totalDebt) * 100) : 0}%</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2">
                  <div
                    className="bg-[#106A3C] h-2 rounded-full"
                    style={{ width: `${totalDebt > 0 ? (debt.balance / totalDebt) * 100 : 0}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-[#106A3C] to-[#0a4a29] rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-[#106A3C]/20">
            <div className="absolute bottom-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-2xl -mr-10 -mb-10" />
  ...
  const navItemClass = (tabId: ActiveTab) =>
    `flex items-center gap-3 px-4 py-3 rounded-2xl relative font-medium transition-colors w-full ${
      activeTab === tabId ? 'bg-[#106A3C]/5 text-[#106A3C]' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
    }`;
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-[#F4F6F8] text-gray-800 font-sans flex overflow-hidden selection:bg-[#106A3C]/20">
        ...
        {/* Sidebar */}
        <aside
          className={`fixed lg:static inset-y-0 left-0 w-[260px] bg-white border-r border-gray-100 z-50 transform transition-transform duration-300 flex flex-col ${
          ...
          <div className="px-4 py-2">
            ...
            <nav className="space-y-1">
              <button onClick={() => { setActiveTab('dashboard'); setIsSidebarOpen(false); }} className={navItemClass('dashboard')}>
                {activeTab === 'dashboard' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-[#106A3C] rounded-r-full" />}
              ...
              <button onClick={() => { setActiveTab('calendar'); setIsSidebarOpen(false); }} className={navItemClass('calendar')}>
                {activeTab === 'calendar' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-[#106A3C] rounded-r-full" />}
                <Calendar size={20} /> Календарь
              </button>
              <button onClick={() => { setActiveTab('analytics'); setIsSidebarOpen(false); }} className={navItemClass('analytics')}>
                {activeTab === 'analytics' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-[#106A3C] rounded-r-full" />}
            ...
            <div className="flex items-center justify-between p-4 lg:px-8 lg:py-6">
              ...
              <div className="flex items-center gap-3 lg:gap-5 ml-auto">
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-[#106A3C] hover:bg-[#0c502d] text-white text-sm font-medium px-4 py-2 rounded-full transition-colors shadow-md shadow-[#106A3C]/20"
                >
                  <PlusCircle size={16} />
                ...
                <div className="flex items-center gap-3 pl-2 lg:pl-5 lg:border-l border-gray-200">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" /> Облако
                    </p>
                  </div>
              ...
              <div className="p-6 md:p-8">
                ...
                <form onSubmit={handleAddDebt} className="space-y-5">
                  <div>
                    ...
                    <input
                      required type="text" value={newDebt.name}
                      onChange={e => setNewDebt({ ...newDebt, name: e.target.value })}
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-gray-900 focus:outline-none focus:border-[#106A3C] focus:bg-white transition-colors"
                      placeholder="Например: Кредитка Тинькофф"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        required type="number" value={newDebt.balance}
                    ...
                    <div>
                      ...
                      <input
                        required type="number" step="0.1" value={newDebt.rate}
                  ...
                  <div>
                    ...
                    <input
                      required type="number" value={newDebt.minPayment}
                  ...
                  <div>
                    ...
                    <input
                      required type="date" value={newDebt.nextPaymentDate}
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      className="w-full bg-[#106A3C] hover:bg-[#0c502d] text-white font-medium py-3.5 rounded-xl transition-colors shadow-lg shadow-[#106A3C]/20"
                    >
                      Добавить в дашборд
                    </button>
                  </div>
